...
# Or like this using the predefined python sum function
# def sum_to(n):
...
# ChatGPT explanation: 
# To handle an arbitrary number of arguments in Python, you can use *args in the function definition. 
# This allows you to pass any number of arguments to the function, and they will be treated as a tuple named args inside the function. 
# Here’s how you can implement the product function to calculate the product of any number of numbers:

def product(*args):
    # Initialize the product with the first number
    result = args[0]
    
    # Multiply each subsequent number with the result
    for num in args[1:]:
        result *= num
...
def basicCalculator(x, y, calculate):
    if calculate == 'add':
        return x + y
